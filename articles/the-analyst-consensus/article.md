...
The structural observation that holds across the named examples is that the incumbents pursuing the workflow-and-governance positioning credibly are doing five things in some combination. They are publishing reference architectures that explicitly name how their platform composes with multiple foundation models rather than competing with any single one. They are emphasizing regulatory durability across banking, insurance, healthcare, government, and telecom, as the dimension on which their position strengthens as agentic AI moves from pilot to production. They are building out governance, audit, identity, and policy enforcement capabilities at the orchestration layer, often in partnership with dedicated AI governance vendors like Credo AI rather than attempting to build everything in-house. They are framing the conversation with consultancies and clients around *the workflow and governance layer your foundation lab engagement needs to compose with* rather than around *the workflow platform that competes with your foundation lab engagement*. And, most recently visible in the early 2026 record, they are accepting that the developer authoring surface is moving into the foundation lab UI and meeting it there with skills libraries published against the open Agent Skills standard, letting the foundation-lab user invoke the workflow platform's authoring capability without leaving Claude or Cursor or Codex, while the runtime, governance, audit, and compliance posture stay in the workflow platform.

The fifth move is the most recent in the public record and the most architecturally consequential. The Agent Skills specification that Anthropic released in December 2025, and that OpenAI adopted shortly after for Codex CLI and ChatGPT, gave the workflow vendors a standardized substrate on which to publish authoring capability the foundation-lab user can invoke directly. Salesforce's `forcedotcom/afv-library` is the company's officially curated agent skills library, optimized for Salesforce's Agentforce Vibes IDE and explicitly compatible with Claude Code, Codex, Cursor, and OpenCode. Agentforce Vibes itself, announced at Salesforce TDX in April 2026, is a Salesforce-hosted browser IDE with Claude Sonnet 4.5 as the default coding model and direct access to the developer's actual org metadata. SAP's developer community has a parallel sap-skills library covering BTP, CAP, Fiori, ABAP, HANA, and Analytics Cloud — community-driven rather than first-party, but the substrate is in place. Pega's Infinity '26.1 release, scheduled for detailed disclosure at PegaWorld 2026 in June, is reported to extend this pattern with a Claude skill that lets a Claude user author Pega applications from inside the foundation-lab UI while the runtime, governance, and compliance posture remain in the Pega platform — itself the latest step in a multi-year evolution that began with multi-model foundation integration in Pega Infinity in 2024 and continued through Predictable AI agents and the Pega Agentic Process Fabric in 2025. The architectural bet across these moves is consistent. The entry point for developer authoring is moving to the foundation lab regardless of what the workflow incumbents do; the durable position is therefore the runtime layer that the foundation labs are structurally not built to be. The bet is unproven. The fact that multiple credible incumbents are making it simultaneously, with publicly inspectable artifacts, is itself the analytically meaningful observation.
